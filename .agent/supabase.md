...
Use when working with Supabase in FIT Automate repos: auth setup, env vars, schema patterns, multi-app configuration. Reference: **.agent/platforms/supabase.yaml** (authoritative config). Cursor skill: `.cursor/skills/supabase/SKILL.md`

---

## Multi-app auth configuration

**Requirement (discovered 2026-03-16 during fit-tools auth rollout):** When multiple apps share one Supabase project, OAuth redirects can land on the wrong app unless:
...
2. **Redirect URLs** in the same URL Configuration include **every app’s `/login` route explicitly**, e.g.:
   - `https://tools.fitautomate.com/login`
   - `https://n8n-insights.fitautomate.com/login`
   - `http://localhost:4001/login` (and any other dev/preview origins used for auth).
...
- `VITE_AUTH_REDIRECT_URL` (optional) — explicit OAuth return URL (e.g. `https://tools.fitautomate.com/login`). Use to avoid redirecting to wrong app; client should enforce same-host when set.
...
**Multi-environment (e.g. n8n-insights):** Optional overrides per env: `SUPABASE_URL_DEV`, `SUPABASE_SERVICE_ROLE_KEY_DEV`, `SUPABASE_DB_SCHEMA_DEV`, and `_DEMO` / `_PROD` variants. Default schema when using schemas: `SUPABASE_DB_SCHEMA` (e.g. `n8n_inventory`).
...
- Localhost bypass: some apps skip auth on localhost (e.g. fit-tools); others require env and show “Supabase not configured” on `/login` when unset.
